...
class PhParser(Parser):
    ...
    def parse_with_retrieved(self, retrieved):
        ...
        calc_input_parameterdata = self._calc.get_inputs(node_type=ParameterData, also_labels=True)
        ...
        if len(parser_opts_query) > 1:
            self.logger.error("Too many ({}) parser_opts found".format(len(parser_opts_query)))
            successful = False
        ...
        if self._calc._OUTPUT_XML_TENSOR_FILE_NAME in list_of_files:
            xml_tensor_file = out_folder.get_abs_path(self._calc._OUTPUT_XML_TENSOR_FILE_NAME)

        # look for dynamical matrices
        dynmat_dir = out_folder.get_abs_path(self._calc._FOLDER_DYNAMICAL_MATRIX)
        dynamical_matrix_list = [
            os.path.join(dynmat_dir, fil)
            for fil in os.listdir(dynmat_dir)
            if os.path.isfile(os.path.join(dynmat_dir, fil)) and
            fil.startswith(os.path.split(self._calc._OUTPUT_DYNAMICAL_MATRIX_PREFIX)[1]) and not fil.endswith(".freq")
        ]
        ...
        out_dict, raw_successful = parse_raw_ph_output(out_file, xml_tensor_file, dynamical_matrix_list)
