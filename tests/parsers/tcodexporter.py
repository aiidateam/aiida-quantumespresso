...
class TestTcodDbExporter(AiidaTestCase):
    ...
    def test_contents_encoding_1(self):
        ...
        self.assertEquals(cif_encode_contents('tabbed\ttext'), ('tabbed=09text', 'quoted-printable'))
        self.assertEquals(cif_encode_contents('angstrom Å'), ('angstrom =C3=85', 'quoted-printable'))
        self.assertEquals(cif_encode_contents('.'), ('=2E', 'quoted-printable'))
        self.assertEquals(cif_encode_contents('?'), ('=3F', 'quoted-printable'))
        ...
        self.assertEquals(
            cif_encode_contents("Å{};a".format("".join("a" for i in range(0, 69)))),
            ('=C3=85aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
             'aaaaaaaaaaaaaaaaaaaaaaaaaaaaa=\n=3Ba', 'quoted-printable'))
        self.assertEquals(cif_encode_contents('angstrom ÅÅÅ'), ('YW5nc3Ryb20gw4XDhcOF', 'base64'))
        self.assertEquals(cif_encode_contents("".join("a" for i in range(0, 2048)))[1], None)
        self.assertEquals(cif_encode_contents("".join("a" for i in range(0, 2049)))[1], 'quoted-printable')
    ...
    def test_cif_structure_roundtrip(self):
        ...
        fd = FolderData()
        with open(fd._get_folder_pathsubfolder.get_abs_path(calc._SCHED_OUTPUT_FILE), 'w') as f:
            f.write("standard output")
            f.flush()

        with open(fd._get_folder_pathsubfolder.get_abs_path(calc._SCHED_ERROR_FILE), 'w') as f:
        ...
        self.assertEquals(values['_tcod_computation_environment'], ['PATH=/dev/null\nUSER=unknown'])
        self.assertEquals(values['_tcod_computation_command'], ['cd 1; ./_aiidasubmit.sh'])
    ...
    def test_pw_translation(self):
        ...
        calc = FakeObject({
            "inp": {
                "parameters": ParameterData(dict={'SYSTEM': {
                    'ecutwfc': 40,
        ...
        self.assertEquals(res, {'_dft_cell_valence_electrons': 10, '_tcod_software_package': 'Quantum ESPRESSO'})
    ...
    def test_export_trajectory(self):
        ...
        expected_tags = [
            ...
            '_atom_site_type_symbol', '_audit_conform_dict_location', '_audit_conform_dict_name',
            '_audit_conform_dict_version', '_audit_creation_method', '_cell_angle_alpha', '_cell_angle_beta',
            '_cell_angle_gamma', '_cell_length_a', '_cell_length_b', '_cell_length_c', '_chemical_formula_sum',
            '_symmetry_equiv_pos_as_xyz', '_symmetry_int_tables_number', '_symmetry_space_group_name_h-m',
            '_symmetry_space_group_name_hall'
        ]

        tcod_file_tags = [
            '_tcod_content_encoding_id', '_tcod_content_encoding_layer_id', '_tcod_content_encoding_layer_type',
            '_tcod_file_content_encoding', '_tcod_file_contents', '_tcod_file_id', '_tcod_file_md5sum',
            '_tcod_file_name', '_tcod_file_role', '_tcod_file_sha1sum', '_tcod_file_uri'
        ]
    ...
    def test_contents_encoding_2(self):
        ...
        def check_ncr(self, inp, out):
            from aiida.tools.dbexporters.tcod import (encode_textfield_ncr, decode_textfield_ncr)
        ...
        def check_base64(self, inp, out):
            from aiida.tools.dbexporters.tcod import (encode_textfield_base64, decode_textfield_base64)
        ...
        def check_gzip_base64(self, text):
            from aiida.tools.dbexporters.tcod import (encode_textfield_gzip_base64, decode_textfield_gzip_base64)
        ...
        check_ncr(self, '<html>&#195;&#133;</html>', '<html>&#38;#195;&#38;#133;</html>')
        ...
        # This one is particularly tricky: a long line is folded by the QP
        # and the semicolon sign becomes the first character on a new line.
        check_quoted_printable(self, "Å{};a".format("".join("a" for i in range(0, 69))),
                               '=C3=85aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
                               'aaaaaaaaaaaaaaaaaaaaaaaaaaaaa=\n=3Ba')
