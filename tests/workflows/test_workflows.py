...
def test_base_unrecoverable_failure(generate_workchain, generate_calc_job_node, generate_inputs, entry_point):
    ...
    assert process.inspect_process() == process.exit_codes.ERROR_UNHANDLED_FAILURE, (
        'The second inspection should return the proper exit code, i.e. the `BaseRestartWorkChain` should abort'
    )
