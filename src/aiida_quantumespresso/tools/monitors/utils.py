...
def parse_accuracy_lines(content):
    ...
    # Match a number (optionally with comma thousands separators) followed by a non-space token (the unit)
    num_unit_pat = re.compile(r'([+-]?(?:\d[\d,]*(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?)\s+(\S+)[\.,;:]*$')
    ...
    for line in content.splitlines():
        ...
        if m:
            ...
            pairs = re.findall(r'([+-]?(?:\d[\d,]*(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?)\s+(\S+)', line)
            if not pairs:
                continue
            num_s = pairs[-1][0]
...
def is_stuck(nums, min_repeats=5):
    """Determine whether the accuracy metric has plateaued.

    Returns True if the last `min_repeats` values in `nums` are all equal to the
    final value *and* are consecutive (i.e. the metric has not moved for the
    last `min_repeats` steps).

    Args:
        nums (array-like): Sequence of numerical values to check.
        min_repeats (int, optional): Number of consecutive identical values
    ...
    return bool(indices[-min_repeats] == len(nums) - min_repeats)
