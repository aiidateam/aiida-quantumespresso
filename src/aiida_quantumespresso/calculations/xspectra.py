...
from aiida.orm import RemoteData, FolderData, SinglefileData, Dict, XyData
...
class XSpectraCalculation(NamelistsCalculation): 
    ...
    # _XANES_FILENAME = 'xanes.dat'
    # _RIXS_FILENAME = 'rixs.dat'
    ...
    def define(cls, spec):
        ...
        spec.input('bz_sampling', valid_type=KpointsData, required=True, help='The K-point sampling to be used for the XSpectra calculation')
        ...
        spec.exit_code(313, 'ERROR_OUTPUT_ABSORBING_SPECIES_WRONG',
            message='The absorbing atom species was set incorrectly, check and ensure that the index value of "xiabs" correctly refers to the ATOMIC SPECIES containing the core-hole (where the index starts from 1).')
    ...
    def prepare_for_submission(self, folder):
        ...
        # This check applies to the in-development version of xspectra, where
        # 'spectroscopy' sets whether to simulate XANES or XRS spectra.
        # First, we check if the spectroscopy field is present (which it will
        # need to be for the XRS code variant) and then sets the output file name
        # explicitly - since this is (annoyingly) fixed by the program and cannot
        # be changed.
        
        # if 'spectroscopy' not in parameters['INPUT_XSPECTRA'].keys():
        #     _Spectrum_FILENAME = 'xanes.dat'
        # elif 'spectroscopy' in parameters['INPUT_XSPECTRA'].keys():
        #     _Spectrum_FILENAME = f"{parameters['INPUT_XSPECTRA']['spectroscopy']}.dat"
