...
class PhCalculation(JobCalculation):
    ...
    def _prepare_for_submission(self, tempfolder, inputdict):
        ...
        if n_parents != 1:
            raise UniquenessError("Input RemoteData is child of {} "
                                  ...
                                  "a single parent".format(n_parents, "" if n_parents == 0 else "s"))
        ...
        try:
            ...
            try:
                ...
                raise InputValidationError("Parent of PhCalculation  does not " "have a default output subfolder")
        #os.path.join(
        ...
        if 'INPUTPH' not in input_params:
            raise InputValidationError(
                "No namelist INPUTPH found in input")  # I cannot decide what to do in the calculation
        ...
        if not restart_flag:  # if it is a restart, it will be copied over
            tempfolder.get_subfolder(self._FOLDER_DYNAMICAL_MATRIX, create=True)
        ...
        if symlink:
            ...
            remote_symlink_list.append((parent_calc_folder.get_computer().uuid,
                                        os.path.join(parent_calc_folder.get_remote_path(), self._get_pseudo_folder()),
        ...
        calcinfo.retrieve_list.append(
            os.path.join(self._OUTPUT_SUBFOLDER, '_ph0', '{}.phsave'.format(self._PREFIX),
    ...
    def _check_valid_parent(self, calc):
        ...
        try:
            ...
            if (((not isinstance(calc, PwCalculation)) and (not isinstance(calc, PhCalculation))) and
                (not isinstance(calc, CopyonlyCalculation))):
                raise ValueError("Parent calculation must be a PwCalculation, a "
                                 "PhCalculation or a CopyonlyCalculation")
        except ImportError:
            if ((not isinstance(calc, PwCalculation)) and (not isinstance(calc, PhCalculation))):
                raise ValueError("Parent calculation must be a PwCalculation or " "a PhCalculation")
    ...
    def use_parent_calculation(self, calc):
        ...
        if len(remotedatas) != 1:
            raise UniquenessError("More than one output remotedata found in " "the parent")
    ...
    def _set_parent_remotedata(self, remotedata):
        ...
        if input_remote:
            raise ValidationError("Cannot set several parent calculation to a " "ph calculation")
    ...
    def _get_pseudo_folder(self):
        ...
        """
        return self.get_attr("pseudo_folder", BasePwCpInputGenerator._PSEUDO_SUBFOLDER)
    ...
    def create_restart(self, force_restart=False, parent_folder_symlink=_default_symlink_usage):
        """
        ...
        if not 'Restart' in c2.label:
            labelstring = c2.label + " Restart of {} {}.".format(self.__class__.__name__, self.pk)
        else:
