...
then
    ...
    verdi setup  --profile=$TEST_AIIDA_BACKEND --db-engine='postgresql_psycopg2' --db-backend="${TEST_AIIDA_BACKEND}" \
        --email='aiida@localhost' --first-name='AiiDA' --last-name='test' --password='secret' --institution='AiiDA Team' \
        --db-host='localhost' --db-port=5432 --db-name="$TEST_AIIDA_BACKEND" --db-username=postgres --db-password='' \
        --repository="/tmp/repository_${TEST_AIIDA_BACKEND}/" --non-interactive

    # Setup the test profile
    verdi setup --profile=test_$TEST_AIIDA_BACKEND --db-engine='postgresql_psycopg2' --db-backend="${TEST_AIIDA_BACKEND}" \
        --email='aiida@localhost' --first-name='AiiDA' --last-name='test' --password='secret' --institution='AiiDA Team' \
        --db-host='localhost' --db-port=5432 --db-name="test_$TEST_AIIDA_BACKEND" --db-username=postgres --db-password='' \
