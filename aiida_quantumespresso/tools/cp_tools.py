...
class SimpleTrajectory(object):
    ...
    def plot_dynamics_data(self, start=0):
        ...
        ((self.df[start:len(self.df)].etot - self.df[start:len(self.df)].econs) /
         (self.df[start:len(self.df)].econs - self.df[start:len(self.df)].econt)).plot(ax=ax2)
...
#
    ...
    for i in range(0, len(pos_all)):
        ...
        trajectories_this.append(
            pos_all[i].get_step(),
            pos_all[i].get_time(),
            cell_all[i].get_array(),
            pos_all[i].get_array(),
            evp[i],
            vel=vel_all)
