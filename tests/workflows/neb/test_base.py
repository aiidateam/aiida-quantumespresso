...
def test_handle_unrecoverable_failure(generate_workchain_neb):
    """Test `NebBaseWorkChain.handle_unrecoverable_failure`."""
    process = generate_workchain_neb(exit_code=NebCalculation.exit_codes.ERROR_NO_RETRIEVED_FOLDER)
    process.setup()

    result = process.handle_unrecoverable_failure(process.ctx.children[-1])
    assert isinstance(result, ProcessHandlerReport)
    assert result.do_break
    assert result.exit_code == NebBaseWorkChain.exit_codes.ERROR_UNRECOVERABLE_FAILURE

    result = process.inspect_process()
    assert result == NebBaseWorkChain.exit_codes.ERROR_UNRECOVERABLE_FAILURE
