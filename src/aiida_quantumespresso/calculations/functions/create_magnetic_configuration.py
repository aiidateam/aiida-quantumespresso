...
def create_magnetic_configuration(
    ...
    atol=lambda: Float(0.5),
    ztol=lambda: Float(0.05),
):
    ...
    * Negative magnetic moments
    ...
    for element in structure.get_symbols_set():
        ...
        for site_list in (zero_sites, pos_sites, neg_sites):
            ...
            sorted_site_list = sorted(site_list, key=lambda x: abs(x[1]), reverse=True)

            sites, magnetic_moments = zip(*sorted_site_list)
            ...
            for site, magnetic_moment in zip(sites[1:], magnetic_moments[1:]):
                if not np.isclose(
                    magnetic_moment,
                    kind_magnetic_moments[current_kind_name],
                    rtol,
                    atol,
                ):
                    kind_index += 1
    ...
    return {
        ...
        'magnetic_moments': Dict(dict=magnetic_configuration),
    }
