...
the BaseRestartWorkChain. The function expects two arguments, a workchain class and a priortity.
The decorator will add the function as a class method to the workchain class and add an ErrorHandler
tuple to the _error_handlers attribute of the workchain. During failed calculation handling the
'inpsect_calculation' outline method will call the '_handle_calculation_failure' which will loop
over all error handler in the '_error_handlers', sorted with respect to the priority in reverse.
If the workchain class defines a '_verbose' attribute and is set to True, a report message will
be fired when the error handler is executed.
...
the function return an ErrorHandlerReport tuple when its conditional was met. If an error was handled
