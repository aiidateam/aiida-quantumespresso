...
class CpParser(Parser):
    ...
    def parse(self, **kwargs):
        ...
        try:
            ...
            #  not self.node._OUTPUT_FILE_NAME in list_of_files:
            successful = False
            new_nodes_tuple = ()
        ...
        try:
            with out_folder.open('{}.pos'.format(self.node._PREFIX)) as posfile:
        ...
        # =============== CELL trajectory ============================
        try:
            with out_folder.open('{}.cel'.format(self.node._PREFIX)) as celfile:
        ...
        try:
            with out_folder.open('{}.vel'.format(self.node._PREFIX)) as velfile:
        ...
        # =============== EVP trajectory ============================
        try:
            matrix = numpy.genfromtxt(os.path.join(out_folder.get_abs_path('.'), '{}.evp'.format(self.nodes._PREFIX)))
