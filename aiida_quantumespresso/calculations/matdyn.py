...
class MatdynCalculation(NamelistsCalculation):
    ...
    def _init_internal_params(self):
        ...
        self._blocked_keywords = [
            ('INPUT', 'flfrq', self._PHONON_FREQUENCIES_NAME),  # output freq.
        ...
        self._internal_retrieve_list = [self._PHONON_FREQUENCIES_NAME, self._PHONON_DOS_NAME]
    ...
    def use_parent_calculation(self, calc):
        ...
        localdatas = [_[1] for _ in calc.get_outputs(also_labels=True) if _[0] == calc.get_linkname_force_matrix()]
        if len(localdatas) == 0:
    ...
    def _prepare_for_submission(self, tempfolder, inputdict):
        ...
        parent_calc_folder = inputdict.get(self.get_linkname('parent_folder'), None)

        if isinstance(parent_calc_folder, SinglefileData):
            self._blocked_keywords.append(('INPUT', 'flfrc', os.path.split(parent_calc_folder.get_file_abs_path())[1]))
        else:
            raise NotImplementedError("Input different from SinglefileData is not supported"
                                      " yet for MatdynCalculation; it is {}".format(type(parent_calc_folder)))
            self._blocked_keywords.append(('INPUT', 'flfrc', Q2rCalculation._FORCE_CONSTANTS_NAME))

        calcinfo = super(MatdynCalculation, self)._prepare_for_submission(tempfolder, inputdict)
        return calcinfo
