...
def launch_calculation(code, calculation, max_num_machines, max_wallclock_seconds, with_mpi, daemon):
    ...
    if daemon:
        new_calc = launch.submit(PpCalculation, **inputs)
        click.echo('Submitted {}<{}> to the daemon'.format(PpCalculation.__name__, new_calc.pk))
    else:
        click.echo('Running a pp.x calculation from parent {}<{}>... '.format(calculation.__class__.__name__,
                                                                              calculation.pk))
        _, new_calc = launch.run_get_node(PpCalculation, **inputs)
        click.echo('PpCalculation<{}> terminated with state: {}'.format(new_calc.pk, new_calc.process_state))
