...
@pytest.mark.parametrize(
    "test_name", ("nonpolarised", "noncollinear", "spinorbit", "numbered_kinds")
)
def test_projwfc(
    generate_projwfc_node, generate_parser, data_regression, tmpdir, test_name
):
    ...
    results, calcfunction = parser.parse_from_node(
        node, store_provenance=False, retrieved_temporary_folder=tmpdir
    )
...
def test_projwfc_spinpolarised(
    generate_projwfc_node, generate_parser, data_regression, tmpdir
):
...
@pytest.mark.parametrize(
    "test_name", ("tdosinboxes", "tdosinboxes_polarized", "tdosinboxes_noncollinear")
)
def test_projwfc_tdosinboxes(
    generate_projwfc_node, generate_parser, data_regression, tmpdir, test_name
):
...
def test_projwfc_no_retrieved_temporary(
    generate_calc_job_node, fixture_localhost, generate_parser
):
    ...
    assert (
        ...
        == node.process_class.exit_codes.ERROR_NO_RETRIEVED_TEMPORARY_FOLDER.status
    )
...
def test_projwfc_xml_failures(
    generate_projwfc_node, generate_parser, tmpdir, test_name, exit_status
):
    """Test ``ProjwfcParser`` fails when the XML is missing."""
    node = generate_projwfc_node(test_name)
    parser = generate_parser("quantumespresso.projwfc")
    _, calcfunction = parser.parse_from_node(
        node, store_provenance=False, retrieved_temporary_folder=tmpdir
    )
