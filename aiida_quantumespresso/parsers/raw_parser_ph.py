...
def parse_raw_ph_output(out_file, tensor_file=None, dynmat_files=[]):
    ...
    final_data = dict(dynmat_data.items() + out_data.items() + tensor_data.items() + parser_info.items())
...
def parse_ph_text_output(lines):
    ...
    for count, line in enumerate(lines):
        if 'q-points for this run' in line:
            try:
                ...
                if ('number_of_qpoints' in parsed_data.keys() and num_qpoints != parsed_data['number_of_qpoints']):
                    parsed_data['warnings'].append("Number q-points found " "several times with different" " values")
                else:
                    parsed_data['number_of_qpoints'] = num_qpoints
            except Exception:
                parsed_data['warnings'].append("Error while parsing number of " "q points.")
            ...
            try:
                ...
                parsed_data['warnings'].append("Error while parsing number of " "atoms.")
    ...
    critical_warnings = {
        'No convergence has been achieved': 'Phonon did not reach end of self consistency',
    ...
    for count, line in enumerate(lines):
        ...
        if any(i in line for i in all_warnings):
            messages = [
                all_warnings[i] if all_warnings[i] is not None else line for i in all_warnings.keys() if i in line
            ]
...
def parse_ph_dynmat(data, lattice_parameter=None, also_eigenvectors=False, parse_header=False):
    """
    ...
    if parse_header:
        ...
        try:
            ...
            try:
                ...
                if abs(alat) < 1.e-5:
                    raise QEOutputParsingError("Lattice constant=0! Probably you are using an "
            ...
            for idx, atom_line in enumerate(data[starting_line:starting_line + num_atoms], start=1):
                ...
                if len(pieces) != 5:
                    raise QEOutputParsingError("Wrong # of elements for one of the atoms: {}, "
                                               "line {}: {}".format(len(pieces), starting_line + idx, pieces))
                try:
                    if int(pieces[0]) != idx:
                        raise QEOutputParsingError("Error with the indices of the atoms: "
            ...
            parsed_data['warnings'].append("Problem parsing the header of the matdyn file! (msg: {}). "
                                           "Storing only the information I managed to retrieve".format(e.message))
            header_dict['warnings'].append("There was some parsing error and this dictionary is "
