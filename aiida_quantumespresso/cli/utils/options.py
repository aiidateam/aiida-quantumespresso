...
STRUCTURE = OverridableOption(
    ...
    help=u'StructureData node.')

PSEUDO_FAMILY = OverridableOption(
    '-p', '--pseudo-family', 'pseudo_family',
    type=click.STRING,
    help=u'Pseudo potential family name.')

KPOINTS_DISTANCE = OverridableOption(
    '-K', '--kpoints-distance', type=click.FLOAT, default=0.5, show_default=True,
    help=u'The minimal distance between k-points in reciprocal space in inverse Ångström.')

KPOINTS_MESH = OverridableOption(
    '-k', '--kpoints-mesh', 'kpoints_mesh', nargs=3, type=click.INT, show_default=True,
...
WITH_MPI = OverridableOption(
    '-i', '--with-mpi', is_flag=True, default=False, show_default=True,
    help=u'Run the calculations with MPI enabled.')
...
ECUTWFC = OverridableOption(
    ...
    help=u'The plane wave cutoff energy in Ry.')
...
HUBBARD_U = OverridableOption(
    ...
    help=u'Add a Hubbard U term to a specific kind.', metavar='<KIND MAGNITUDE>...')

HUBBARD_V = OverridableOption(
    '-V', '--hubbard-v', nargs=4, multiple=True, type=click.Tuple([int, int, int, float]),
    help=u'Add a Hubbard V interaction between two sites.', metavar='<SITE SITE TYPE MAGNITUDE>...')

HUBBARD_FILE = OverridableOption(
    '-H', '--hubbard-file', 'hubbard_file_pk', type=types.DataParamType(sub_classes=('aiida.data:singlefile',)),
...
STARTING_MAGNETIZATION = OverridableOption(
    ...
    help=u'Add a starting magnetization to a specific kind.', metavar='<KIND MAGNITUDE>...')

SMEARING = OverridableOption(
    '-S', '--smearing', nargs=2, default=(None, None), type=click.Tuple([unicode, float]),
    help=u'Add smeared occupations by specifying the type and amount of smearing.', metavar='<TYPE DEGAUSS>')
