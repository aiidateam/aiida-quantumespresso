...
The new features are support for electron-phonon calculations, DOS interpolations with `matdyn.x` and LDOS and total PDOS parsing from the `projwfc.x` code.
...
This support release for v4.X provides several new features, critical bug fixes for the `PdosWorkChain` and a large number of improvements that are currently on the `main` branch.
