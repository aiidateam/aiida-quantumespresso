...
def import_qeinput(fname):
    ...
    def generate_cell(ibrav, parameters):
        ...
        if ibrav == 1:
            ...
            cellAlat[2][2] = parameters[2] * np.sqrt(1 + 2 * np.cos(alpha) * np.cos(beta) * np.cos(gamma) - (
                np.cos(beta) * np.cos(beta)) - (np.cos(alpha) * np.cos(alpha)) - (np.cos(gamma) * np.cos(gamma))) / (
                    np.sin(gamma))
    ...
    while line:
        ...
        if cell_parameters.search(line) and nat > 0 and ibrav == 0 and cell is None:
            ...
            if celldm is not None and a is not None:
                raise Exception("Cannot declare both celldm and A, B, C, cosAB, cosAC, cosBC (a={}, celldm={})".format(
                    a, celldm))
