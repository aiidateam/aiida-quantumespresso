...
from aiida_quantumespresso.parsers.raw_parser_pw import (parse_pw_xml_output, parse_pw_text_output,
    ...
    def parse_with_retrieved(self, retrieved):
        ...
        for i in range(num_images):
            ...
            xml_file = os.path.join(
                out_folder.get_abs_path('.'), self._calc._PREFIX + '_{}'.format(i + 1), self._calc._PREFIX + '.save',
            ...
            pw_out_file = os.path.join(out_folder.get_abs_path('.'), self._calc._PREFIX + '_{}'.format(i + 1), 'PW.out')
            try:
            ...
            skip_keys = [
                'forces', 'atomic_magnetic_moments', 'atomic_charges', 'lattice_vectors_relax',
        ...
        try:
            mep_file = os.path.join(out_folder.get_abs_path('.'), self._calc._PREFIX + '.dat')
            mep = numpy.loadtxt(mep_file)
        except Exception:
            self.logger.warning("Impossible to find the file with image energies " "versus reaction coordinate.")
            mep = numpy.array([[]])

        try:
            interp_mep_file = os.path.join(out_folder.get_abs_path('.'), self._calc._PREFIX + '.int')
