...
#@options.CALCULATION(required=True, type=types.CalculationParamType(sub_classes=('aiida.calculations:quantumespresso.pw',)))
...
def cli(code, calculation, max_num_machines, max_wallclock_seconds, with_mpi, daemon):
    ...
    if daemon:
        ...
        click.echo('Running a dos.x calculation from parent {}<{}>... '.format(calculation.__class__.__name__, calculation.pk))
