...
def test_pw_failed_computing_cholesky_stdout(
    fixture_localhost, generate_calc_job_node, generate_parser, generate_inputs
):
    """Test the parsing of a calculation that failed during cholesky factorization, with the error in stdout.

    In this test the stdout is incomplete, and the XML is missing completely. The stdout contains
    the relevant error message.
    """
    name = 'failed_computing_cholesky_stdout'
    entry_point_calc_job = 'quantumespresso.pw'
    entry_point_parser = 'quantumespresso.pw'
