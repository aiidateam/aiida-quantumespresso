...
Take care when setting inputs via the `overrides`!
Currently, the `get_builder_from_protocol` method will simply ignore any `overrides` that are not used, for example when you make a typo, see: [https://github.com/aiidateam/aiida-quantumespresso/issues/1134](https://github.com/aiidateam/aiida-quantumespresso/issues/1134)
...
:::{dropdown} Minimal working example
```python
...
Finally, you can also directly pass your inputs to the engine by preparing all of them in an `inputs` dictionary:

```
engine.run(PwBaseWorkChain, **inputs)
```
This is shown in the dropdown below.
...
Instead, we typically want to `sumbit` the process to the AiiDA _daemon_, which will take care of its execution.
